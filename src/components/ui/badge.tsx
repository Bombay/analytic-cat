  ...
  'inline-flex items-center rounded-full border border-zinc-200 px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:ring-offset-2 dark:border-zinc-800 dark:focus:ring-zinc-300',
  {
    variants: {
      variant: {
        ...
        dashed: 'text-zinc-950 dark:text-zinc-50 border-dashed border-zinc-950',
      },
    },
    defaultVariants: {
      variant: 'default',
...
function Badge({ className, variant, children, closable = false }: BadgeProps) {
  return (
    <div className={cn(badgeVariants({ variant }), className)}>
      {children}
      {closable && <AiOutlineClose className="ml-1 hover:cursor-pointer" />}
    </div>
  )
}
